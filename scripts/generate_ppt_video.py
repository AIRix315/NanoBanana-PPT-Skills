...
def generate_ppt_video_from_images(
    ...
    if video_provider == "enterprise-video":
        print(f"Enterprise Model: {enterprise_model}")
    ...
    materials_generator = VideoMaterialsGenerator(
        ...
        provider_type=video_provider,
        enterprise_model=enterprise_model,
    )

    # Prepare content contexts
    content_contexts = [
        f"Transition from slide {i + 1} to slide {i + 2}" for i in range(num_slides - 1)
    ]
    ...
    print(
        f"  Videos: {materials_result['success_count']} success, "
...
def generate_video_viewer(
    ...
    template_path = str(
        Path(__file__).parent.parent / "assets" / "templates" / "video_viewer.html"
    )
...
def validate_inputs(args: argparse.Namespace) -> bool:
    ...
    if not os.path.exists(args.prompts_file):
        ...
        print(
            f"     'Analyze images in {args.slides_dir}, generate transition prompts,"
        )
...
def main() -> None:
    ...
    try:
        result = generate_ppt_video_from_images(
            ...
            enterprise_model=args.enterprise_model,
        )
