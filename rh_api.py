...
class RunningHubClient:
    ...
    def generate_image(
        ...
        if model not in IMAGE_ENDPOINTS:
            raise ValueError(f"不支持的模型: {model}，支持的模型: {list(IMAGE_ENDPOINTS.keys())}")
    ...
    def _poll_task(
        ...
        poll_url = f"{RUNNINGHUB_BASE_URL}/task/{task_id}"
        ...
        while True:
            ...
            try:
                ...
                if status == "SUCCESS":
                    ...
                    raise RunningHubTaskError(
                        f"{task_type}失败: {error_msg}"
                    )
        ...
        try:
            ...
            if response.status_code != 200:
                raise RunningHubError(
                    f"下载图像失败，状态码: {response.status_code}"
                )
...
def get_rh_client() -> RunningHubClient:
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", choices=["v2", "pro"], default="pro", help="图像模型")
